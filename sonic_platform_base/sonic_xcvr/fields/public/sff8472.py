...
class TempField(NumberRegField):
    def __init__(self, name, offset, *fields, **kwargs):
        kwargs["deps"] = [consts.DIAG_MON_TYPE_FIELD, consts.T_SLOPE_FIELD, consts.T_OFFSET_FIELD]
        super(TempField, self).__init__(name, offset, *fields, **kwargs)

    def decode(self, raw_data, **decoded_deps):
        diag_mon_type = decoded_deps.get(consts.DIAG_MON_TYPE_FIELD)
        int_cal = diag_mon_type & 0x20
        ext_cal = diag_mon_type & 0x10
...
class VoltageField(NumberRegField):
    def __init__(self, name, offset, *fields, **kwargs):
        kwargs["deps"] = [consts.DIAG_MON_TYPE_FIELD, consts.V_SLOPE_FIELD, consts.V_OFFSET_FIELD]
...
class TxPowerField(NumberRegField):
    def __init__(self, name, offset, *fields, **kwargs):
        kwargs["deps"] = [consts.DIAG_MON_TYPE_FIELD, consts.TX_PWR_SLOPE_FIELD, consts.TX_PWR_OFFSET_FIELD]
...
class RxPowerField(NumberRegField):
    def __init__(self, name, offset, *fields, **kwargs):
        kwargs["deps"] = [consts.DIAG_MON_TYPE_FIELD, consts.RX_PWR_0_FIELD, consts.RX_PWR_1_FIELD, 
